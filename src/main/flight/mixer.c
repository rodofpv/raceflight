...
void mixTable(void)
{
    ...
    if (rollPitchYawMixRange > throttleRange) {
        ...
        if ((mixReduction > (mixerConfig->airmode_saturation_limit / 100.0f)) && (IS_RC_MODE_ACTIVE(BOXAIRMODE) || IS_RC_MODE_ACTIVE(BOXALWAYSSTABILIZED))) {
    ...
    for (i = 0; i < motorCount; i++) {
        ...
        // Motor stop handling
        if (feature(FEATURE_MOTOR_STOP) && ARMING_FLAG(ARMED) && !feature(FEATURE_3D) && !IS_RC_MODE_ACTIVE(BOXAIRMODE)&& !IS_RC_MODE_ACTIVE(BOXALWAYSSTABILIZED)) {
